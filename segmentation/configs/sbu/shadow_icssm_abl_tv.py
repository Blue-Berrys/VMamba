# 消融实验 Row 1: IC-SSM only (no BG-SIR, no Tversky)
...
model = dict(
    ...
    decode_head=dict(
        ...
        boundary_kernel=5,
        tversky_loss_weight=0.5,   # ← 关闭 Tversky
        tversky_alpha=0.3,
        tversky_beta=0.7,
        use_bg_sir=False,          # BG-SIR 关闭, 只测 Tversky
        dropout_ratio=0.1,
        num_classes=2,
        align_corners=False,
        loss_decode=[
...
train_cfg = dict(_delete_=True, type='IterBasedTrainLoop',
                 max_iters=30000, val_interval=5000)
...
optim_wrapper = dict(
    ...
    optimizer=dict(type='AdamW', lr=0.00003,
...
custom_hooks = [dict(type='ProgressiveTrainingHook',
                     ...
                     stage3_iters=30000, log_stage_switch=True)]

log_processor = dict(by_epoch=False)
work_dir = './work_dirs/shadow_icssm_abl_tv'
