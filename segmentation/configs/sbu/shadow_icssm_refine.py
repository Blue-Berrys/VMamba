# MMSegmentation 配置: IC-SSM + BG-SIR + Tversky + SBU-Refine 精标注训练
# ============================================================================
#
# SBU-Refine 策略:
#   - 同一批 SBU 训练图像 (lssd1.jpg ... lssd4085.jpg), 完全无域偏移
#   - mask 由 SILT 精修 (软标注 0-255), 用 >=128 阈值二值化
#   - 消除原始 SBU 标注中边界标注误差 → 更干净的训练信号
#
# 从 BG-SIR 最优 checkpoint 续训 (BER=3.45%, iter 10000)
# 目标: 通过更准确的标注进一步降低 BER (<3.0%)
# ============================================================================
...
# ============================================================================
# 数据: SBU 图像 + SBU-Refine 精修 mask
#   train: 图像来自 SBU, mask 来自 SBU-Refine (>=128 阈值二值化)
#   val:   原始 SBU test (638 张, 保持与所有方法的公平对比)
# ============================================================================

train_pipeline = [
    dict(type='LoadImageFromFile'),
    # RefineAnnTransform: 加载 SBU-Refine 软标注并阈值化 (>=128 → shadow=1)
...
work_dir = './work_dirs/shadow_icssm_refine'
